...
import { FiMail, FiLock, FiShield, FiUsers, FiSettings } from 'react-icons/fi'
...
const AdminLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        // Ensure user is Admin or SuperAdmin role
      ...
      <div className="max-w-md w-full">
        <div className="card-3d bg-white p-8 rounded-2xl shadow-3d">
          <div className="text-center mb-8">
            ...
            <p className="text-text-muted">Full system access and user management</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="bg-background-light p-4 rounded-lg border-2 border-secondary">
              <div className="flex items-start gap-2">
                <FiSettings className="text-primary mt-0.5" size={18} />
                <div className="text-xs text-text-muted">
                  <p className="font-medium text-text-primary mb-1">Admin Access Includes:</p>
                  <ul className="list-disc list-inside space-y-1">
                    <li>User management (Agents & Finance)</li>
                    <li>Trip management & dispute resolution</li>
                    <li>System-wide reports & audit logs</li>
                  </ul>
                </div>
              </div>
            </div>
          ...
          <div className="mt-6 pt-6 border-t border-secondary">
            <p className="text-xs text-center text-text-muted mb-2">
              Demo: Use email containing 'admin' or 'superadmin' (e.g., admin@test.com)
            </p>
            <div className="flex gap-2 justify-center text-xs">
              <Link to="/login/finance" className="text-primary hover:underline">Finance Login</Link>
